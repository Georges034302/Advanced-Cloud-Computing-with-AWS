...
echo "---"
SUBNET_LIST=$(echo "$SUBNET_IDS" | xargs | tr ' ' ',')
...
LAUNCH_TEMPLATE_OUTPUT=$(aws ec2 create-launch-template \
  ...
  --version-description "Initial version with web server" \
...
# Wait for initial instances to launch
sleep 30
...
## Step 7 – Create Scheduled Scaling Actions
...
aws autoscaling describe-scaling-activities \
  --auto-scaling-group-name "$ASG_NAME" \
  --max-records 10 \
  --query 'Activities[*].{Time:StartTime,Description:Description,StatusCode:StatusCode}' \
  --output table \
  --region "$REGION"
